export function readySampleBody(count, sparkIcon) {
  return `<div class="sample-canvas sample-canvas--empty"><div><span class="empty-state__icon">${sparkIcon}</span><h3>准备生成 ${count} 页 HTML-PPT</h3><p>Agent 会按需阅读 Skills，并生成一个带自身翻页交互、可独立打开的 HTML-PPT 包。</p><button class="btn btn--primary" data-action="generate_sample">生成 HTML-PPT</button></div></div>`;
}
...
function historyBody(history, selectedHash, escapeHtml) {
  ...
  const cards = history.map((revision) => {
    ...
    const branchAction = selected
      ...
      : "";
    return `<article class="sample-revision-card${selected ? " is-selected" : ""}"><button class="sample-revision-card__selector" type="button" data-sample-revision="${escapeHtml(revision.revision_hash)}" aria-pressed="${selected}" ${selected ? "disabled" : ""}><span class="sample-revision-card__number">R${revision.revision}</span><span class="sample-revision-card__content"><span class="sample-revision-card__title">修订 ${revision.revision}${selected ? '<span class="badge badge--success">当前</span>' : ""}${revision.status === "approved" ? '<span class="badge badge--info">已确认</span>' : ""}</span>${feedback}<small>${escapeHtml(revisionDate(revision.created_at))} · ${slideCount} 页${packageInfo ? ` · ${packageInfo.file_count} 个文件` : ""}</small></span></button>${branchAction}</article>`;
  }).join("");
...
export function sampleBody(sample, escapeHtml, options = {}) {
  ...
  const feedback = `<form class="sample-feedback" id="sample-feedback-form"><div class="field"><label for="sample-feedback">修改意见</label><textarea class="input" id="sample-feedback" name="feedback" maxlength="4000" required placeholder="例如：封面减少文字，把核心数字放大；保留当前翻页方式。" aria-describedby="sample-feedback-help"></textarea><small id="sample-feedback-help">AI 会读取当前指针对应的完整文件包，并生成一个以当前版本为父节点的新修订。</small><div class="field-error" id="sample-feedback-error" role="alert"></div></div><div class="sample-feedback__actions"><button class="btn btn--secondary" type="button" data-action="regenerate_sample">重新生成</button>${approved ? "" : '<button class="btn btn--secondary" type="button" data-action="approve_sample">确认 HTML-PPT</button>'}<button class="btn btn--primary" type="submit">让 AI 修改</button></div></form>`;
  ...
  return { body: `${completion}${preview}${feedback}${historyMarkup}`, status };
}
